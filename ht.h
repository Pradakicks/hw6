...
struct Prober {
  ...
  void init(HASH_INDEX_T start, HASH_INDEX_T m, const KeyType& key) {
    (void)
        key;  // avoid unused argument warnings since base class doesn't use key
    start_ = start;
    m_ = m;
    numProbes_ = 0;
  }
...
struct LinearProber : public Prober<KeyType> {
  HASH_INDEX_T next() {
    // completed 4/22/24
    // If we are back at the start that means we have done a full circle
...
class HashTable {
 ...
 private:
  ...
  size_t numOfUsedSpace;
  size_t numOfRemovedItems;
  double resizeAlpha_;
...
HashTable<K, V, Prober, Hash, KEqual>::HashTable(double resizeAlpha,
  ...
  // TODODODODODODODODODODODOD
  // Thus, when a HashTable is constructed it should start with a size of 11.
  // The client will supply a loading factor, alpha, at construction, and before
  // inserting any new element, you should resize (to the next larger size in
  // the list above) if the current loading factor is at or above the given
  // alpha.
...
bool HashTable<K, V, Prober, Hash, KEqual>::empty() const {
  return numOfUsedSpace - numOfRemovedItems == 0;
}

template <typename K, typename V, typename Prober, typename Hash,
          typename KEqual>
size_t HashTable<K, V, Prober, Hash, KEqual>::size() const {
  return numOfUsedSpace - numOfRemovedItems;
}

template <typename K, typename V, typename Prober, typename Hash,
          typename KEqual>
void HashTable<K, V, Prober, Hash, KEqual>::insert(const ItemType& p) {
  if (double(numOfUsedSpace) / CAPACITIES[mIndex_] >= resizeAlpha_) {
    this->resize();
  }
  HASH_INDEX_T h = this->probe(p.first);
  if ((npos == h)) {
    // return;
    throw std::logic_error("No Keys");
    return;
  } else if (nullptr == table_[h]) {
...
template <typename K, typename V, typename Prober, typename Hash,
          typename KEqual>
HASH_INDEX_T HashTable<K, V, Prober, Hash, KEqual>::probe(
